...
include("connect.php");
   $date=$_GET['startdate'];
   $date2=$_GET['untildate'];
   if($date==""||$date2==""){
   	echo "Δεν έδωσες καποιο search query";
    }
   else{

	 $sql="SELECT  id_event ,id, category , datetime, name, description, pic, owner FROM events WHERE date(datetime) BETWEEN '$date' AND '$date2' ORDER BY datetime ASC"; 
